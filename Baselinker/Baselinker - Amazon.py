''' Automatyczne wystawianie aukcji Amazon na platformie Baselinker '''
...
class Baselinker():
    ...
    def amazon(self):
        try:
            self.wait.until(EC.presence_of_all_elements_located((By.ID, "menu_amazon")))
            self.driver.find_element_by_id("menu_amazon").click()
            # self.driver.find_element_by_xpath('//a[@href="/amazon_sell.php"]').click()
        except Exception as error:  # pylint: disable=broad-except
            print(error)
    ...
    def filter_amount(self):
        try:
            ...
            self.driver.find_element_by_id('products_quantity_from').send_keys('3')
    ...
    def set_category(self,category):
        ...
        try:
            ...
            self.driver.find_element_by_id('category-add-button').click()
            time.sleep(3)
            self.driver.find_element_by_xpath("//*[text()='Wystaw zaznaczone']").click()
            time.sleep(3)
        except Exception as error:  # pylint: disable=broad-except
            print(error)
            return
        ...
        try:
            prices = self.driver.find_elements_by_xpath('//*[@class="field_price autonumeric"]')
            time.sleep(len(prices))
            for price in prices:
                
                p = price.get_attribute("value")
                price.send_keys(p[:-3])
            time.sleep(3)
